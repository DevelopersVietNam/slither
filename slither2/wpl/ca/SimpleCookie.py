...
####
# 
# $Id: SimpleCookie.py,v 1.1 2005/03/07 23:13:51 gkt Exp $
#  by Timothy O'Malley <timo@bbn.com>
#
#  Cookie.py is an rewrite for the old nscookie.py module.
...
#    requires the re and cPickle modules introduced in Python-1.5.
#
...
two methods of the Cookie:

  Cookie.net_setfunc()    Takes in an encoded string and returns a value
  Cookie.user_setfunc()  Takes in a value and returns the encoded string
...
class BaseCookie(UserDict):
    ...
    def net_setfunc(self, strval):
        """net_setfunc(STRING)
        Called prior to setting a cookie's value from the network
        representation.  The STRING is the value read from HTTP
        header.
    ...
    def user_setfunc(self, strval):
        """user_setfunc(STRING)
        Called prior to setting a cookie's value from the dictionary
        representation.  The STRING is the value being assigned.
        Override this function to modify the behavior of cookies.
        """
        return strval
    # end user_setfunc
    
    def __init__(self, input=None, net_setfunc=None, user_setfunc=None):
        if net_setfunc:
            # Backwards compatible way to override method
            self.net_setfunc  = lambda s,x: net_setfunc(x)
        if user_setfunc:
            # Backwards compatible way to override method
            self.user_setfunc = lambda s,x: user_setfunc(x)
        UserDict.__init__(self)
...
class SimpleCookie(BaseCookie):
    ...
    """
    def net_setfunc(self, val):
        return _unquote( val )
    def user_setfunc(self, val):
